...
class AsyncFileManager:
    ...
    async def archive_file(source_file, dest_file):
        try:
            async with aiofiles.open(source_file, 'rb') as f_in, aiofiles.open(dest_file, 'wb') as f_out:
                while True:
                    buf = await f_in.read(8192)
                    if not buf:
                        break
                    await f_out.write(buf)
...
async def create_superuser():
    ...
    email = typer.prompt("Введите email для суперпользователя")
    password = getpass.getpass("Введите пароль (ввод скрыт): ")
    confirm_password = getpass.getpass("Повторите пароль (ввод скрыт): ")

    if password != confirm_password:
        typer.echo("Пароли не совпадают. Попробуйте снова.", err=True)
        raise typer.Exit(code=1)

    async for session in get_async_session():
        user_manager = await get_user_manager_without_db_session(db_session=session)
        ...
        try:
            user = await user_manager.create(superuser_data)
            typer.echo(f"Суперпользователь {user.email} успешно создан.")
        except UserAlreadyExists:
            typer.echo(f"Пользователь с email {email} уже существует.", err=True)
        except Exception as e:
            typer.echo(f"Ошибка при создании суперпользователя: {e}", err=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(sys.argv) > 1 and sys.argv[1] in ('restore_database', 'dump_db', 'create_superuser'):
            command = sys.argv[1]
            try:
